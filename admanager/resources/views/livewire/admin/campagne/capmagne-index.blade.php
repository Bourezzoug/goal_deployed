<div wire:init="loadItems">
    ...
    <!-- component -->
    <div class="m-5">
      @can('create' , \App\Models\Campagne::class)
      ...
      @endcan
      @can('deleteAll' , \App\Models\Campagne::class)
...
<div class="overflow-hidden rounded-lg border border-gray-200 shadow-md m-5">
    ...
    <thead class="bg-gray-50">
        <tr>
            ...
            <th scope="col" class="px-6 py-4 font-medium text-gray-900">Date Début</th>
            <th scope="col" class="px-6 py-4 font-medium text-gray-900">Date Fin</th>
            <th scope="col" class="px-6 py-4 font-medium text-gray-900">Status</th>
            <th scope="col" class="px-6 py-4 font-medium text-gray-900">Clients</th>
    ...
    <tbody class="divide-y divide-gray-100 border-t border-gray-100">
        ...
        <td class="pl-6 py-4">
                ...
                <input type="checkbox" wire:model="selectedCampagnes" value="{{ $campagne->id }}" class="bg-neutral-50 border-neutral-200 ">
        ...
        <td class="px-6  py-4">
            <div class="flex gap-2">
              ...
              >
              {{ date('D jS M Y',strtotime($campagne->date_debut)) }} 
              </span>
            </div>
        </td>
        <td class="px-6  py-4">
            <div class="flex gap-2">
              <span
                class="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-1 w-32 text-xs font-semibold text-indigo-600"
              >
              {{ date('D jS M Y',strtotime($campagne->date_fin)) }} 
              </span>
            </div>
        </td>
        <td class="px-6 py-4">
        ...
        <td class="px-6 py-4">
          ...
          >
              <span class="h-1.5 w-1.5 rounded-full bg-gray-600"></span>
              {{ $campagne->client->nom . " " . $campagne->client->prenom  }}
          </span>
        </td>
        <td class="px-6  py-4">
            <div class="flex gap-2">
              <span
                class="inline-flex items-center gap-1 rounded-full bg-indigo-50 px-2 py-1 w-32 text-xs font-semibold text-indigo-600"
              >
              {{ date('D jS M Y',strtotime($campagne->created_at)) }} 
              </span>
            </div>
          </td>
          <td class="px-6 py-4">
            ...
            <div class="flex gap-4">
                @can('update', $campagne)
                <div class="cursor-pointer" wire:click="selectedItem('update',{{ $campagne->id }})"
                              class="px-2">
            ...
            @can('view', $campagne)
                <div class="cursor-pointer" wire:click="selectedItem('show',{{ $campagne->id }})"
                              class="px-2">
            ...
            @can('delete', $campagne)
                <div class="cursor-pointer" wire:click="selectedItem('delete',{{ $campagne->id }})"
                              class="px-2">
    ...
    <livewire:admin.campagne.campagne-create :clients="$clients" />
    <livewire:admin.campagne.campagne-update  :clients="$clients"/>
    <livewire:admin.campagne.campagne-show  />
    <livewire:admin.campagne.campagne-delete/>
</div>
