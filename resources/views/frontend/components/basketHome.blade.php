...
<main class="mt-10">
    ...
    <div class="block lg:flex lg:space-x-2 px-2 md:px-14 lg:p-0 mt-10 gap-3">
        ...
        <div class="mb-4 flex items-center justify-between space-x-1">
            <h5 class="font-semibold text-lg capitalize p-2 rounded text-white mb-2 font-Roboto-c" style="background-color:{{ $basketHero->category->color }}">Basket</h5>
            <div class="flex-1 border-t-2 border-gray-200"></div>
            <a href="/basket" class="font-semibold text-gray-800 px-1 mb-2 text-sm md:text-base  font-Roboto-c">Voir tous les articles <i class="fa-solid fa-arrow-right"></i></a>
        </div>

        @forelse ($basket as $article)
        ...
        <div class="w-full lg:w-[30%] sm:px-3">
                ...
                <!--        {{-- <article class="flex flex-col gap-4 lg:flex-row lg:gap-6 mb-8">-->
                <!--            <a href="sport/{{ $article->category->slug . '/' .$article->slug }}" class="group relative block h-56 md:h-72 w-full shrink-0 self-start overflow-hidden rounded-lg bg-gray-100 shadow-lg lg:h-40 lg:w-40">-->
                <!--              <img src="{{ $article->image }}" loading="lazy" alt="" class="absolute inset-0 h-full w-full object-cover object-center transition duration-200 group-hover:scale-110" />-->
                <!--            </a>-->
                    
                <!--            <div class="flex flex-col gap-2">-->
                    
                <!--              <h2 class="font-Roboto-condensed font-bold" style="font-size:16px">-->
                <!--                <a href="sport/{{ $article->category->slug . '/' .$article->slug }}" class="transition duration-100  text-base">{{ $article->title }}</a>-->
                <!--              </h2>-->
                <!--              <span class="text-sm text-gray-400 font-Lato">{{ strftime('%e %B %Y', strtotime($article->created_at)) }}</span>-->
