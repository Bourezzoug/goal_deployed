...
<main class="mt-10">
    ...
    <div class="block lg:flex lg:space-x-2 px-2 md:px-14 lg:p-0 mt-10 gap-3">
        ...
        <div class="mb-4 flex items-center justify-between space-x-1">
            <h5 class="font-semibold text-lg capitalize p-2 rounded text-white mb-2 font-Roboto-c" style="background-color:#902A23">Divers</h5>
            <div class="flex-1 border-t-2 border-gray-200"></div>
            <a href="/divers" class="font-semibold text-gray-800 px-1 mb-2 text-sm md:text-base font-Roboto-c">Voir tous les articles <i class="fa-solid fa-arrow-right"></i></a>
        </div>
        <div class="grid grid-cols-12 gap-3">


        @forelse ($diverHome as $key => $footarticle)

        @if($key != 2 && $key != 3)
        <div class="col-span-12 md:col-span-4 relative">
            <div class="relative">
                <img src="{{ $footarticle->image }}" class="h-[220px] w-full object-cover" alt="">
                <div class="absolute left-0 bottom-0 w-full h-full z-10"
                style="background-image: linear-gradient(180deg,transparent,rgba(0,0,0,.7));"></div>
            </div>
            <h3 class="font-Roboto absolute top-5 left-5 rounded text-white font-bold" style="padding:4px 10px 3px;font-size:11px;background:{{ $footarticle->category->color }};z-index:1000;"><a href="{{ $footarticle->category->slug }}">{{ $footarticle->category->name }}</a></h3>
            <span class="font-Roboto" style="font-size:12px;color:#000000C4;padding:0px 15px">{{ \Carbon\Carbon::parse($footarticle->created_at)->isoFormat('dddd Do MMMM YYYY', 'LL', 'fr') }}</span>
            <h2 class="font-Roboto-c font-bold " style="font-size:16px;line-height:1.2;padding:5px 15px">
                <a href="sport/{{ $footarticle->category->slug . '/' .$footarticle->slug }}">{{ $footarticle->title }}</a>
            </h2>
        </div>
        @else
        <div class="col-span-12 md:col-span-4 h-[300px] relative">
            <img src="{{ $footarticle->image }}" class="h-full w-full object-cover" alt="">
            <div class="absolute left-0 bottom-0 w-full h-full z-10"
            style="background-image: linear-gradient(180deg,transparent,rgba(0,0,0,.7));"></div>
            <h3 class="font-Roboto absolute top-5 left-5 rounded text-white font-bold" style="padding:4px 10px 3px;font-size:11px;background:{{ $footarticle->category->color }};z-index:1000;"><a href="{{ $footarticle->category->slug }}">{{ $footarticle->category->name }}</a></h3>
            <h2 class="font-Roboto-c font-bold p-[15px] absolute bottom-5 left-5 text-white z-50" style="font-size:16px;line-height:1.2">
                <a href="sport/{{ $footarticle->category->slug . '/' .$footarticle->slug }}">{{ $footarticle->title }}</a>
            </h2>
            <span class="font-Roboto absolute bottom-3 left-5 text-gray-100 z-50" style="font-size:12px;padding:0px 15px">{{ \Carbon\Carbon::parse($footarticle->created_at)->isoFormat('dddd Do MMMM YYYY', 'LL', 'fr') }}</span>
        </div>
        @endif
        ...
        <div class="w-full lg:w-[30%] px-3 pt-5">
                ...
                <div class="pt-1 pb-8">
                    <h2 class="post-header relative text-sm font-semibold font-Roboto-c"><span class="bg-main-color text-white rounded-full py-2 px-3">Les plus vues</span></h2>
                </div>
            <div class="xs:max-w-xs">
                ...
                <div class="flex sm:px-1 py-4">
                    ...
                    <div class="flex flex-col flex-grow">
                        <h2 class="font-Roboto font-bold" style="font-size:15px;">
...
<main class="mt-10 mb-10">
    ...
    <div class="block lg:flex lg:space-x-2 px-2 md:px-14 lg:p-0 mt-10 gap-3">
        ...
        <div class="mb-4 flex items-center justify-between space-x-1">
            <h5 class="font-semibold text-lg capitalize p-2 rounded text-white mb-2 font-Lato" style="background-color:#854d0e">Auto Moto</h5>
            <div class="flex-1 border-t-2 border-gray-200"></div>
            <a href="/auto-moto" class="font-semibold text-gray-800 px-1 mb-2 text-sm md:text-base font-Lato">Voir tous les articles <i class="fa-solid fa-arrow-right"></i></a>
        </div>
        <div class="grid grid-cols-12 gap-3">


        @forelse ($autoHome as $key => $article)

        @if($key != 0 && $key != 5)
        <div class="col-span-12 md:col-span-4 relative">
                        <h3 class="font-Roboto absolute top-5 left-5 rounded text-white font-bold" style="padding:4px 10px 3px;font-size:11px;background:{{ $article->category->color }};z-index:1000;"><a href="{{ $article->category->slug }}">{{ $article->category->name }}</a></h3>
            <div class="relative">
                <img src="{{ $article->image }}" class="h-[220px] w-full object-cover" alt="">
                <div class="absolute left-0 bottom-0 w-full h-full z-10"
                style="background-image: linear-gradient(180deg,transparent,rgba(0,0,0,.7));"></div>
            </div>
            <span class="font-Roboto" style="font-size:12px;color:#000000C4;padding:0px 15px">{{ \Carbon\Carbon::parse($article->created_at)->isoFormat('dddd Do MMMM YYYY', 'LL', 'fr') }}</span>
            <h2 class="font-Roboto-c font-bold " style="font-size:16px;line-height:1.2;padding:5px 15px">
                <a href="sport/{{ $article->category->slug . '/' .$article->slug }}">{{ $article->title }}</a>
            </h2>
        </div>
        @else
        <div class="col-span-12 md:col-span-4 h-[300px] relative">
            <img src="{{ $article->image }}" class="h-full w-full object-cover" alt="">
            <div class="absolute left-0 bottom-0 w-full h-full z-10"
            style="background-image: linear-gradient(180deg,transparent,rgba(0,0,0,.7));"></div>
            <h2 class="font-Roboto-c font-bold p-[15px] absolute bottom-5 left-5 text-white z-50" style="font-size:16px;line-height:1.2">
                <a href="sport/{{ $article->category->slug . '/' .$article->slug }}">{{ $article->title }}</a>
            </h2>
            <span class="font-Roboto absolute bottom-3 left-5 text-gray-100 z-50" style="font-size:12px;padding:0px 15px">{{ \Carbon\Carbon::parse($article->created_at)->isoFormat('dddd Do MMMM YYYY', 'LL', 'fr') }}</span>
        </div>
        @endif
            ...
            <div class="2xl:mx-auto 2xl:container pt-2">
                <form id="bannerClickForm-{{ $bannerTennisRight->id }}" action="{{ route('banner.click', ['id' => $bannerTennisRight->id]) }}" method="POST" target="_blank">
                    ...
                    <button type="submit" class=" bg-none border-none ">
                        <img id="banner-{{ $bannerTennisRight->id }}" src="http://localhost:8001{{ $bannerTennisRight->image }}" data-banner-id="{{ $bannerTennisRight->id }}" class="w-[300px] md:h-[300px] object-fill" alt="">
                    </button>
                </form>
            </div>
            @endif
